...
const Signup = () => {
  ...
  const onVerifySubmit = async (e) => {
    ...
    try {
      ...
      toast.success('Account verified! Welcome to Income Tracker.');
      navigate('/dashboard');
    } catch (error) {
  ...
  return (
    <div className="min-h-screen flex bg-surface font-inter overflow-x-hidden">
      ...
      {/* Auth Side */}
      <div className="flex-1 flex items-center justify-center p-8 py-20 relative overflow-y-auto font-inter">
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="mb-12">
                <button 
                  ...
                  className="inline-flex items-center text-xs font-bold text-accent uppercase tracking-widest mb-8 hover:gap-2 transition-all"
                >
                  <ArrowLeftIcon className="h-4 w-4 mr-2" />
                  Back to Registration
                </button>
                <h2 className="text-4xl font-black text-text-primary tracking-tight mb-3">Security Protocol</h2>
                <p className="text-text-muted font-medium">Enter the 10-character code sent to <span className="text-text-primary font-bold">{email}</span>.</p>
              </div>

              <form onSubmit={onVerifySubmit} className="space-y-8">
                <div className="space-y-2 group text-center">
                  <div className="relative">
                    <ShieldCheckIcon className="absolute left-5 top-1/2 -translate-y-1/2 h-6 w-6 text-text-muted group-focus-within:text-accent transition-colors" />
                    <input
                      type="text"
                      value={otpCode}
                      onChange={(e) => setOtpCode(e.target.value)}
                      required
                      placeholder="ENTER 10-CHAR OTP"
                      maxLength={10}
                      className="w-full pl-14 pr-6 py-6 bg-secondary/50 border border-border rounded-2xl focus:ring-2 focus:ring-accent/20 focus:border-accent outline-none transition-all font-black text-2xl tracking-[0.2em] text-center text-text-primary placeholder:text-text-muted/20 placeholder:tracking-normal placeholder:text-sm"
                    />
                  </div>
                </div>
                ...
                <button
                  type="submit"
                  disabled={isLoading || otpCode.length !== 10}
                  className="w-full bg-accent text-white py-5 text-sm font-black rounded-xl hover:bg-accent-dark transition-all shadow-premium active:scale-95 disabled:opacity-50 flex items-center justify-center gap-3 uppercase tracking-widest"
                >
                  {isLoading ? (
                    <>
                      <div className="animate-spin h-5 w-5 border-2 border-white/20 border-t-white rounded-full"></div>
              ...
              <div className="mt-12 text-center">
                <p className="text-sm font-medium text-text-muted">
                  ...
                  <button onClick={onSignupSubmit} className="font-black text-accent hover:underline uppercase tracking-tighter">
                    Resend Security Key
                  </button>
                </p>
              </div>
